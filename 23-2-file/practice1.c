...
int input_book_list(BOOK *book_list, int *total_num_book);
...
int main() {
  ...
  while (1) {
    ...
    printf("7. input_book_list.txt 에서 책 정보 가져오기 \n");
    printf("당신의 선택은 : ");
    scanf("%d", &user_choice);
    if (user_choice == 1) { /* 책을 새로 추가하는 함수 호출 */
      ...
      input_book_list(book_list, &num_total_book);
    }
  }
  free(book_list);
  return 0;
...
int input_book_list(BOOK *book_list, int *nth) {
    
    FILE *fp = fopen("input_book_list.txt", "r");
    ...
    while (fgetc(fp) != EOF) {
        ...
        if (mode == 0) {
            get_line(fp, book_list[*nth].book_name);
            mode++;
        } else if (mode == 1) {
            get_line(fp, book_list[*nth].auth_name);
            mode++;
        } else if (mode == 2) {
            get_line(fp, book_list[*nth].publ_name);
            mode++;
        } else if (mode == 3) {
            get_line(fp, c);
            if (c[0] == 'Y') book_list[*nth].borrowed = 1;
            else book_list[*nth].borrowed = 0;
            mode++;
        } else {
            (*nth)++;
            mode = 0;
